...
async function mainScript(initialTex)
{
  ...
  function binStr(val) { return '0b' + val.toString(2).padStart(8, '0'); }
  ...
  function writeChar(asciiCode)
  {
    if (asciiCode.length == 1 || asciiCode == 'Backspace' || asciiCode == 'Enter')
      curKey = asciiCode;
  }
  ...
  document.addEventListener('keydown', (event) => {
    if (guiControls.tool == 'text') {
      if (event.code == 'Backquote') {
        guiControls.tool = 'select'; // exit text mode
      } else
        writeChar(event.key);
      return;
    }
  ...
  var positionAttribLocation = gl.getAttribLocation(setupProgram,
                                                    'vertPosition'); // 0 these positions are the same for every program,
  // since they all use the same vertex shader
  var texCoordAttribLocation = gl.getAttribLocation(setupProgram,
                                                    'vertTexCoord'); // 1
  ...
  gl.bindFramebuffer(gl.FRAMEBUFFER, frameBuff_0);
  gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture_0, 0);
  ...
  // load textures
  var imgElement = await loadImage('resources/ASCII2.png');
  ...
  gl.uniform2i(gl.getUniformLocation(setupProgram, 'resolution'), sim_res_x, sim_res_y);

  gl.useProgram(logicProgram);
  gl.uniform2f(gl.getUniformLocation(logicProgram, 'texelSize'), texelSizeX, texelSizeY);
  gl.uniform2i(gl.getUniformLocation(logicProgram, 'resolution'), sim_res_x, sim_res_y);

  gl.useProgram(userInputProgram);
  gl.uniform2f(gl.getUniformLocation(userInputProgram, 'texelSize'), texelSizeX, texelSizeY);
  gl.uniform2i(gl.getUniformLocation(userInputProgram, 'resolution'), sim_res_x, sim_res_y);
  ...
  function draw()
  {
    ...
    if (selection) {
      gl.uniform4i(gl.getUniformLocation(currentDisplayProgram, 'selection'), selectionMinX, selectionMinY, selectionMaxX, selectionMaxY);
    } else {
      gl.uniform4i(gl.getUniformLocation(currentDisplayProgram, 'selection'), -1, -1, -1, -1);
    }

    gl.uniform2f(gl.getUniformLocation(currentDisplayProgram, 'aspectRatios'), sim_aspect, canvas_aspect);
    gl.uniform3f(gl.getUniformLocation(currentDisplayProgram, 'view'), viewXpos, viewYpos, viewZoom);
  ...
  async function loadShader(nameIn)
  {
    ...
    if (shaderSource) {
      ...
      return new Promise(async (resolve) => { resolve(shader); });
    }
  }
